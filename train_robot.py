...
# from robot_dataloader import RadarDataset
from robot_dataloader import RadarDataset_offline
...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='MSL_train_robot_pointnet_10class_tailed_10words', help='experiment root')
...
def main(args):
    ...
    full_dataset = RadarDataset_offline(data_path=args.data_path_offline, N=args.num_point, F=args.num_frame)
    ...
    testDataLoader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=0, drop_last=False)
    ...
    shutil.copy('./train_robot.py', str(exp_dir))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            correct = pred_choice.eq(target.long().data).cpu().sum()
            mean_correct.append(correct.item() / float(points.size()[0]))
            mean_acc_loss.append(loss.item() / float(points.size()[0]))
            loss.backward()
            optimizer.step()
            global_step += 1
