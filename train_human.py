...
# from human_dataloader import RadarDataset
from human_dataloader import RadarDataset_offline
...
def parse_args():
    ...
    parser.add_argument('--num_category', type=int,  help='training on ModelNet10/40')
    ...
    parser.add_argument('--argument', type = bool, default=False, help='use data argumentation')

    parser.add_argument('--process_data', action='store_true', default=False, help='save data offline')
    parser.add_argument('--data_path_online', default='data/human/flb', help='data path from online')
    ...
    parser.add_argument('--train_size', type=float, help='proportion of the dataset to include in the train split')
...
def test(model, loader, num_class=40):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        ...
        points = points.clone().detach().float()
    ...
    return instance_acc, class_acc, np.concatenate(targets), np.concatenate(preds)
...
def main(args):
    ...
    full_dataset = RadarDataset_offline(data_path=args.data_path_offline, N=args.num_point, F=args.num_frame, if_argu=args.argument)
    ...
    train_dataset, test_dataset, remain_dataset = random_split(full_dataset, [train_size, test_size, remain_size], generator=torch.Generator().manual_seed(42))
    ...
    testDataLoader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=0, drop_last=False)

    '''MODEL LOADING'''
    num_class = args.num_category
    model = importlib.import_module(args.model)
    shutil.copy('./models/%s.py' % args.model, str(exp_dir))
    shutil.copy('./human_dataloader.py', str(exp_dir))
    shutil.copy('train_human.py', str(exp_dir))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            instance_acc, class_acc, targets, preds = test(classifier.eval(), testDataLoader, num_class=num_class)

            metrics = Metrics(targets, preds, num_class, log_dir, args.log_dir)
